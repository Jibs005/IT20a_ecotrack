...
class EcoDashboard extends JFrame {
   ...
   private JPanel buildOverviewPanel() {
    ...
    RoundedPanel headerCard = new RoundedPanel(12, Color.WHITE);
    headerCard.setLayout(new BorderLayout());
    headerCard.setBorder(new EmptyBorder(12,12,12,12));
    JLabel title = new JLabel("Overview");
    title.setFont(new Font("Segoe UI", Font.BOLD, 18));
    headerCard.add(title, BorderLayout.WEST);

    p.add(headerCard, BorderLayout.NORTH);
   ...
   JTextArea info = new JTextArea(
        ...
        "- Class Rankings shows electricity per classroom .\n" +
        "- Detailed Logs contain searching + sorting (.\n\n" +
    ...
    info.setForeground(new Color(60,70,80));
...
summaryCard.setLayout(new GridLayout(4, 1));
summaryCard.setBorder(new EmptyBorder(16, 16, 16, 16));

JLabel sumTitle = new JLabel("Summary of Electricity Usage");
sumTitle.setFont(new Font("Segoe UI", Font.BOLD, 15));
sumTitle.setForeground(new Color(20, 70, 120));
...
for (String room : currentData.keySet()) {
    int val = currentData.get(room).electricity;
    total += val;

    if (val > highest) { highest = val; highRoom = room; }
    if (val < lowest) { lowest = val; lowRoom = room; }
}

int avg = currentData.size() > 0 ? total / currentData.size() : 0;

summaryCard.add(new JLabel("• Total Electricity (All Rooms): " + total + " kWh"));
summaryCard.add(new JLabel("• Highest Consumption: " + highRoom + " — " + highest + " kWh"));
summaryCard.add(new JLabel("• Lowest Consumption: " + lowRoom + " — " + lowest + " kWh"));
summaryCard.add(new JLabel("• Average Per Classroom: " + avg + " kWh"));

p.add(summaryCard, BorderLayout.SOUTH);
